...
public class TinyGP {
    ...
    public static final int
            ...
            COS = 116,
            FSET_START = ADD,
            FSET_2ARG_END = DIV,  //last 2 argument function, start of 1 argument functions
            FSET_END = DIV;
    ...
    /** Save population safely (any char values allowed) */
    public static void savePopulationToFile(String filename) {
        try (BufferedWriter writer = Files.newBufferedWriter(Paths.get(filename))) {
            for (char[] individual : population) {
                // Convert char[] -> String -> bytes -> Base64
                String encoded = Base64.getEncoder()
                        .encodeToString(new String(individual).getBytes("UTF-8"));
                writer.write(encoded);
                writer.newLine();
            }
            System.out.println("Population saved to " + filename);
        } catch (IOException e) {
            System.err.println("Error saving population: " + e.getMessage());
        }
    }

    /** Load population safely (decode Base64 lines) */
    public static void loadPopulationFromFile(String filename) {
        try (BufferedReader reader = Files.newBufferedReader(Paths.get(filename))) {
            population = reader.lines()
                    ...
                    .toArray(char[][]::new);
            System.out.println("Population loaded from " + filename);
        } catch (IOException e) {
    ...
    void simplify_con(char[] prog, EvaluationContext ctx) {
        // simplify the individual
        // evaluate operations between constants
        // put operations and evaluation result into separate arrays (operations and numbers)
        // 0 in operations means it's a number
    ...
    double run_rec() {  //evaluates simplified individual
        int primitive = operations[PC++];
        if (primitive < FSET_START){  //it's a number
            return numbers[PC - 1];
        }
        // it's a variable or an operation
        switch (primitive) {
            case ADD -> {
                return run_rec() + run_rec();
            }
            case SUB -> {
                return run_rec() - run_rec();
            }
            ...
            default -> {
                return variables[primitive - 1];  //it's a variable
            }
        }
    }

    static double run_iter(double[] stack) {
        int sp = 0;
        int pc = length - 1;
    ...
    static double run_iter_con(double[] stack, EvaluationContext ctx) {
        int sp = 0;
        int pc = ctx.length - 1;
    ...
    double fitness_function_no_simp_seq_rec(char [] prog) {
        double result, actual, fit = 0.0;
        for (int i = 0; i < fitnesscases; ++i ) {
            System.arraycopy(targets[i], 0, variables, 0, variables.length);
            program = prog;
            PC = 0;
            result = run();
    ...
    double fitness_function_simp_seq_iter(char [] prog) {
        double result, actual, fit = 0.0;
        simplify(prog);
    ...
    double fitness_function_simp_seq_rec(char [] prog) {
        double result, actual, fit = 0.0;
        simplify(prog);
        for (int i = 0; i < fitnesscases; ++i ) {
    ...
    char [][] create_random_pop(int n, int depth, double [] fitness) {
        ...
        for ( i = 0; i < n; i ++ ) {
            ...
            fitness[i] = fitness_function_simp_seq_iter( pop[i] );
        }
        return( pop );
    }
    ...
    void evolve() {
        ...
        for ( gen = 1; gen < GENERATIONS; gen ++ ) {
            ...
            for ( indivs = 0; indivs < POPSIZE; indivs++ ) {
                ...
                // switch between different fitness functions
                if (useSimplify) {
                    if (useIterative) {
                        newfit = fitness_function_simp_seq_iter(newind);
                    } else {
                        newfit = fitness_function_simp_seq_rec(newind);
                    }
                } else {
                    newfit = fitness_function_no_simp_seq_rec(newind);
                }
                ...
                if (indivs % 1000 == 0) {
                    ...
                    System.out.print("["+"■".repeat(progress)+" ".repeat(100 - progress)+"] "
                            +progress+"%  "
                            + df.format(time/1_000_000_000.0) +"s\r");
                }
            }
            System.out.println();
            ...
            for (int t = 0; t < NUM_THREADS; t++) {
                ...
                executor.submit(() -> {
                    ...
                    for (int i = start; i < end; i++) {
                        ...
                        newfit = fitness_function_simp_con_iter(newind, ctx);
    ...
    public static boolean useSimplify;
    public static boolean useIterative;

    public TinyGP(String fname, long s, char[][] population, boolean useSimplify, boolean useIterative) {
        // Allowed boolean values 00, 10, 11
        fitness =  new double[POPSIZE];
        seed = s;
        if (seed >= 0)
            rd.setSeed(seed);
        ...
        for (int i = 0; i < FSET_START; i++)
            x[i]= (maxrandom-minrandom)*rd.nextDouble()+minrandom;
        variables = new double[varnumber];
        TinyGP.population = population;
        for (int i = 0; i < POPSIZE; i++) {
            fitness[i] = fitness_function_simp_seq_iter(population[i]);
        }
        TinyGP.useSimplify = useSimplify;
        TinyGP.useIterative = useIterative;
    }
